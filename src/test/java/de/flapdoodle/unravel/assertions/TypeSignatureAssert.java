...
public class TypeSignatureAssert extends AbstractAssert<TypeSignatureAssert, TypeSignature> implements CommonAsserts {
    ...
    public TypeSignatureAssert isAtLeast(JavaVersion javaVersion) {
        if (javaVersion.compareTo(actual.javaVersion()) > 0) {
            failWithMessage(propertyDescription("javaVersion") + " expected to be at least <%s> but was <%s>",
                    javaVersion, actual.javaVersion());
        }
        return this;
    }
    ...
    public TypeSignatureAssert superClass(Class<?> type) {
        ...
        Assertions.assertThat(actual.superClazz().get().value()).describedAs(propertyDescription("superClazz"))
                .isEqualTo(Classnames.nameOf(type));
        return this;
    }
    ...
    public TypeSignatureAssert interfaces(String... name) {
        if (name.length == 0) {
            ...
            Assertions.assertThat(actual.interfaces().map(ATypeName::value))
                    .describedAs(propertyDescription("interfaces")).contains(name);
        }
        return this;
    }
    ...
    public TypeSignatureAssert innerClasses(Consumer<TypeSignaturesAssert> consumer) {
        consumer.accept(TypeSignaturesAssert.assertThatTypeSignatures(actual.innerClasses())
                .describedAs(propertyDescription("innerClasses")));
        return this;
    }
    //
